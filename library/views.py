...
def viewbook_view(request):
    ...
    if query:
        books = models.Book.objects.filter(
            Q(name__icontains=query) | Q(author__icontains=query) | Q(isbn__icontains=query)
        )
    else:
    ...
    return render(request, 'library/viewbook.html', {'books': books, 'query': query, 'filter_by': filter_by})
...
def issuebook_view(request):
    ...
    print("Form initialized with fields:", form.fields)  # Debugging line
    if request.method == 'POST':
...
def reserve_book_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            reservation.student = models.StudentExtra.objects.get(user=request.user)
            reservation.save()
            return HttpResponseRedirect('/reservations/')
    else:
        form = forms.ReservationForm()

    return render(request, 'library/reserve_book.html', {'form': form})

@login_required(login_url='studentlogin')
def view_reservations(request):
    student = models.StudentExtra.objects.get(user=request.user)
    reservations = models.Reservation.objects.filter(student=student)
    return render(request, 'library/view_reservations.html', {'reservations': reservations})
